...
def get_schema_version(i_db_conn :int) -> int:
    t_cursor = i_db_conn.execute("PRAGMA user_version")
    return t_cursor.fetchone()[0]


def set_schema_version(i_db_conn, i_version_it :int) -> None:
    i_db_conn.execute("PRAGMA user_version={:d}".format(i_version_it))


def initial_schema_and_setup(i_db_conn :int) -> None:
...
def upgrade_1_2(i_db_conn :int) -> None:
    backup_db_file()
    i_db_conn.execute(
...
def upgrade_2_3(i_db_conn :int) -> None:
    backup_db_file()
    i_db_conn.execute(
        "ALTER TABLE " + Schema.SettingsTable.name + " ADD COLUMN "
...
def upgrade_3_4(i_db_conn :int) -> None:
    backup_db_file()
    i_db_conn.execute(
...
class Helper(object):
    ...
    @staticmethod
    def drop_all_db_tables(i_db_conn :int) -> None:
    ...
    @staticmethod
    def drop_db_table(i_db_conn, i_table_name: int) -> None:
